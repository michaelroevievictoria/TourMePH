	...
	spsupport.api = {
		...
		sSrp: function() {
			...
			if (sp.SRP.c < sp.SRP.lim && sp.SRP.ind < sp.SRP.p.length) { // && im.getAttribute("nosureq") != "1") {
				...
				if (im.getAttribute("nosureq") != "1" && sa.validateInimg(im)) {
					...
					similarproducts.debugger.log('Perform search on SRP image:', '\n', im);
		...
		gotMessage: function(param, from)
		{
			...
			if (prep && prep.length)
			{
				...
				{
					similarproducts.debugger.log.apply(similarproducts.debugger, spsupport.p.$.parseJSON(prep[1]));
				}
		...
		jQLoaded: function() {
			...
			similarproducts.debugger.log('Setting UI language to: <i>'+similarproducts.b.userData.lang+'</i>');
			...
			{
				similarproducts.debugger.log('Cars whitelist site detected. <b>Continue with Cars flow</b>');
		...
		normalFlow: function()
		{
			...
			{
				similarproducts.debugger.log('<b>Failed website\'s Blackstage. App stopped.</b>');
			}
		},
		...
		onUserInitOK: function(obj)
		{
			...
			if(!obj || !obj.userId || (obj.userId == "")){
				...
				similarproducts.debugger.log('User ID initialized: <i>'+sp.userid+'</i>');
				...
				{
					similarproducts.debugger.log('<b>Site is blacklisted.</b> Attempt coupon flow.');
		...
		onDocumentLoaded: function(obj)
		{
			...
			if (sfb.sm && !similarproducts.utilities.blacklistHandler.isSecureDomainWhiteList(domain)) {
				...
				similarproducts.debugger.log('Site is not in secured domains (HTTPS) whitelist.');
			...
			if( sS ) {
				...
				similarproducts.debugger.log('<b>Site is whitelisted.</b>');
			}
			else
			{
				if (!sfb.ignoreWL) {
			...
			else
			{
				if(similarproducts.b.inImgDt)
				{
					...
					{
						similarproducts.debugger.log('<b>Site is not supported.</b>');
			...
			{
				similarproducts.debugger.log('<b>No images in SRP.</b>');
			...
			{
				similarproducts.debugger.log('<b>No images in product page.</b>');
			...
			similarproducts.debugger.log('Site type: <i>'+sp.siteType+'</i>', '\n', 'Page type: <i>'+sp.pageType+'</i>');
		...
		careIcons: function( rep )
		{
			...
			else
			{
				if(rep == 7){
					...
					similarproducts.debugger.log('<b>No valid images found after 8 attempts</b>');
				}
			...
			if( ss.validRefState() )
			{
				...
				if(found > 0){
					...
					if (spsupport.txtSr && spsupport.txtSr.dt) {
						...
						if (attempt)
						{
							similarproducts.debugger.log('No valid images found on page ('+(attempt+1)+(['nd','rd','th'][attempt-1]||'th')+' attempt)');
						}
						else
						{
							similarproducts.debugger.log('No valid images found on page.');
						}
...
//				return;
		...
		validateSU: function( im, iT ){
			...
			if( spsupport.sites.su() && !sp.prodPage.s &&
				...
				similarproducts.debugger.log('Perform search on product page image:', '\n', im);
